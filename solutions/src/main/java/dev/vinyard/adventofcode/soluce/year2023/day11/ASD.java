...
public class ASD {
    ...
    public record Root(List<Galaxy> galaxies) {
        ...
        public long calculateSumOfShortestPathBetweenEveryPairOfGalaxies() {
            ...
            empty.stream().forEach(l -> System.out.println(l.getP1() + " " + l.getP2()));
            ...
            while (!stack.isEmpty()) {
                ...
                for (Galaxy g : stack) {
                    ...
                    sum += (long) (Math.abs(line.x1 - line.x2) + Math.abs(line.y1 - line.y2) + empty.stream().filter(line::intersectsLine).count());
                }
            }
